...
function PreTitle({ data }: { data: FichaI | undefined }) {
    return (
        <div className="pre-title">
            ...
            <p className='mb-[10px] text-lg'>GRADO {data ? <u>{data.level}</u> : "____"} SECCION {data ? <u>{data.section}</u> : "____"} TURNO {data ? <u>{data.turno}</u> : "___"}</p>
        </div>
    );
}
...
function StatusInfo({ data }: { data: StatusI | undefined }) {
    return (
        <div style={{ display: "inline-block", border: "1px solid black", padding: "4px", width: "100%", textAlign: "justify" }}>
            {
                data ? <>
                    <p>EL ESTUDIANTE PRACTICA ALGUNA ACTIVIDAD EXTRACURRICULAR SI<u>{data.preferAct ? "x" : " "}</u> NO<u>{data.preferAct ? " " : "x"}</u> CUAL {data.preferAct ? <u>{data.preferAct}</u> : "_____________________"}:</p>
